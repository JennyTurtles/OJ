...
//    }
    ...
    @PostMapping("/findAllShools")
    public RespBean findAllShools() {
        List<PMSchool> schools = schoolServiceI.findAllShools();
        return RespBean.ok("查询所有学校成功", schools);
    }
...
//
    ...
    public RespBean getSchoolByName(@RequestBody PMSchool pmschool){
        // 返回前台的json数据
        PMSchool school = schoolServiceI.getSchoolByName(pmschool.getName());
        if (school != null) {
            return RespBean.ok("查询学校成功", school);
        } else {
            return RespBean.error("学校不存在");
        }
    }
//
