...
public class ExamCacheManager {
	...
	private static ExamCacheManager instance;
	private static Object lock = new Object();

	private ExamCacheManager() {
		// 这个根据配置文件来，初始BaseCache而已;
		Properties prop = new Properties();
		InputStream in = ExamCacheManager.class
				.getResourceAsStream("/cache.properties");
		try {
			prop.load(in);
			String validTime = prop.getProperty("examproblemsValidTime").trim();
			cache = new BaseCache("Exam", Integer.parseInt(validTime));
		} catch (IOException e) {
