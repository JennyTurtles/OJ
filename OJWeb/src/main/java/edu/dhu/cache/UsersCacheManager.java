...
public class UsersCacheManager {
	...
	private UsersCacheManager() {
		// 这个根据配置文件来，初始BaseCache而已;
		Properties prop = new Properties();
		InputStream in = UsersCacheManager.class
				.getResourceAsStream("/cache.properties");
		try {
			prop.load(in);
			String validTime = prop.getProperty("usersValidTime").trim();
			cache = new BaseCache("Users", Integer.parseInt(validTime));
		} catch (IOException e) {
	...
	public static UsersCacheManager getInstance() {
		if (instance == null) {
			synchronized (lock) {
				if (instance == null) {
